...
- **BUG: forced re-sign-in after idle (still recurring)** — user reports being forced to sign back in after some idle period (duration unconfirmed). NOTE: a known cause was fixed in 1.3.0 build 3 (transient refresh failures no longer sign out — only an explicit 401/403 rejection does; see `APIClient.refresh`/`RefreshOutcome`). First step: confirm whether this still reproduces on **1.3.0 (3) or later** — if the report predates that build, it may already be resolved. If it still happens on (3)+, suspects in order: (1) the refresh-token rotation lost-response edge case already backlogged below (client keeps a now-revoked token after a dropped response → legit 401 → sign-out); (2) the 90-day refresh token itself being revoked/expired/rotated out from under a second device (refresh tokens are single-use, so two devices sharing an account can invalidate each other); (3) a manual-URLRequest method (the 20+ that bypass the get/post helpers) hitting a 401 without attempting refresh. Capture: how long idle before it happens, whether on app cold-launch vs foreground-from-background, whether `TokenStore.shared.refreshToken` is non-nil at the time, and what `/auth/refresh` returns.
