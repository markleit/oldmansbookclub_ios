...
- **BUG: sender name flips display-name → nickname while sending** — The optimistic bubble for a message you send shows your **display name**, then when the server echo / refetch replaces it the name flips to your **nickname** (server DTOs use `Nickname ?? DisplayName`, e.g. `BooksController` message projection line ~163). Looks sloppy. Fix: build the optimistic `senderName` as `TokenStore.shared.nickname ?? TokenStore.shared.displayName` so it matches what the server echoes — `BookViewModel.sendMessage` (text) and `enqueueAndSendMedia` / `restorePendingMediaBubbles` (media), all currently `TokenStore.shared.displayName ?? ""` (lines ~201, 227, 309).
- **Predownload messages on notification arrival** — When a push arrives, prefetch the affected book's new message(s) — and ideally warm the image/media cache — so opening the chat from the notification is instant instead of showing a load. Approaches: a `content-available` silent push to trigger a background fetch + cache-warm keyed by the notification's bookId; and/or kick a prefetch in `didReceive` / `willPresent`. Mind the iOS background-execution budget and dedupe with the foreground `load()`. Pairs with the existing deep-link-by-bookId flow.
