...
- **Perf: chat load/scroll slow with many audio messages** — a chat heavy with voice messages loads and/or scrolls sluggishly. Profile and find the per-voice-bubble cost: candidates include per-row work in `VoiceMessageBubble` (PlaybackProgressStore lookups, `readers(of:)` read-receipt computation, `displayFraction`), AVPlayer/asset setup, transcription button state, or LazyVStack re-rendering all voice bubbles on each `audio.progress` tick during playback (every voice bubble observes `AudioPlayerService`). Instrument with Instruments (Time Profiler / SwiftUI) before optimizing; likely wins: avoid re-rendering non-playing bubbles on progress ticks, and memoize read-receipt lookups.
