...
- **BUG (CONFIRMED on 1.3.0 build 3, live): forced re-sign-in after idle** — reproduces on build 3, which already contains the transient-failure fix — so that fix did NOT cover this. On build 3, sign-out only happens when `/auth/refresh` returns 401/403 (`.rejected`); the server returns 401 only for empty/not-found/**revoked**/expired tokens. After a short idle, expiry is ruled out → the token is **revoked or not-found**, i.e. a **refresh-token rotation** failure. Leading mechanism: refresh tokens are single-use/rotated (`AuthController.Refresh` revokes old + issues new). After idle, **Azure App Service has often cold-started** (idle → unloaded), so the first `/auth/refresh` is slow; the client's 15s `session` timeout fires → build 3 treats it as `.transient` and KEEPS the old token — but the server already committed the rotation and revoked it. The next refresh sends the dead token → 401 → sign-out. The transient handling can't save this case because the token was consumed server-side but the replacement never reached the client. **Fix direction (server-side, robust):** make rotation tolerant of lost responses — store the successor on each `RefreshToken` row (e.g. `ReplacedByTokenHash`/grace window) and, when a token revoked within the last ~60s is presented, return its already-issued successor instead of 401. **Client mitigations:** bump the refresh request timeout well above Azure cold-start (e.g. 30-45s); optionally persist the new token before the old is considered consumed. Secondary suspects if the above doesn't fully resolve: a manual-URLRequest method (the 20+ bypassing the get/post helpers) 401ing without a refresh attempt; multi-device single-use invalidation (less likely — each sign-in issues its own token row).
...
- **Refresh-token rotation lost-response edge case** (follow-up to the idle-signout fix) — refresh tokens are single-use/rotated server-side (`AuthController.Refresh` revokes the old + issues a new one). If the network drops *after* the server commits the rotation but *before* the client persists the new token, the client keeps the now-revoked token and the next refresh legitimately 401s → sign-out. Narrow window, but to close it: add a short server-side grace (accept the immediately-previous token for ~30-60s and return the already-rotated successor), or persist the new token before consuming the old. Lower priority than the transient-failure fix already shipped.
