...
- **BUG: forced re-sign-in after idle** — user reports being forced to sign back in after some idle period (duration unconfirmed). Observed on the **currently live App Store build**, which PREDATES the idle-signout fix (that fix shipped in 1.3.0 build 3, still in review as of 2026-06-10 — not yet downloadable). So this is very likely the already-fixed cause: transient refresh failures used to clear tokens and sign out; build 3 makes `APIClient.refresh` only sign out on an explicit 401/403 (`RefreshOutcome`). **Action: re-verify once 1.3.0 (3) is approved and installed.** If it STILL reproduces on (3)+, suspects in order: (1) the refresh-token rotation lost-response edge case backlogged below (client keeps a now-revoked token after a dropped response → legit 401 → sign-out); (2) the 90-day refresh token revoked/expired/rotated out from under a second device (refresh tokens are single-use, so two devices on one account can invalidate each other); (3) a manual-URLRequest method (the 20+ that bypass the get/post helpers) hitting a 401 without attempting refresh. Capture: idle duration before it happens, cold-launch vs foreground-from-background, whether `TokenStore.shared.refreshToken` is non-nil at the time, and what `/auth/refresh` returns.
